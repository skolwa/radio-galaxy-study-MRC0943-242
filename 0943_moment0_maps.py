# S.N. Kolwa (2017)
# Purpose: 
# 0943_moment0_maps.py
# - Construct narrow-band images for all spectral features
# - Make use of continuum-subtracted or non-continuum-subtracted cube
...
for spec_feat,lam1,lam2 in zip(spec_feat,lam1,lam2):
	...
	##spec load and save then mpdaf open like before
	cont_cube 		= sc.SpectralCube.read(fname,hdu=1,formt='fits')
	cont_cube.write(fname, overwrite=True)
	...
	pl.annotate(s='', xy=(98.5,0.5), xytext=(74.5,0.5), arrowprops=dict(arrowstyle='<->',ec='red'))
	pl.text(82.,2.,'80 kpc',fontsize=9,color='red')
	...
	if spec_feat != 'Lya':
		figure = ax1.imshow(gs_img,cmap='gray_r',origin='lower',interpolation='nearest',	transform=ax1.get_transform(wcs),vmin=0.5*vmin,vmax=vmax)
		...
		figure = ax1.imshow(gs_img,cmap='gray_r',origin='lower',interpolation='nearest',	transform=ax1.get_transform(wcs),vmin=vmin,vmax=0.09*vmax)
